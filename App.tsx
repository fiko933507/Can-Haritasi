  ...
  const submit = async () => {
    ...
    try {
      const result = mode === 'register'
        ? await authClient.signUp.email({ email: normalizedEmail, password, name: name.trim(), callbackURL: 'canharitasi://' })
        : await authClient.signIn.email({ email: normalizedEmail, password, callbackURL: 'canharitasi://' });
...
function MapScreen({ reports, position, onHelp, onModerated }: { reports: NearbyReport[]; position: Position | null; onHelp: (id: string) => Promise<void>; onModerated: () => Promise<void> }) {
  ...
  return <View style={{ flex: 1, backgroundColor: '#17382F' }}>
...
function ProfileScreen({ user, reportCount }: { user: { name?: string | null; email?: string | null }; reportCount: number }) {
  ...
  return <ScrollView contentContainerStyle={styles.scroll}>
    ...
    <View style={styles.levelCard}><Image source={require('./assets/volunteer-dog-real.webp')} resizeMode="contain" style={styles.profileAnimal} /><Text style={styles.levelSmall}>GERÇEK HESAP</Text><Text style={styles.levelTitle}>Topluluk üyesi</Text><Text style={styles.levelNumber}>{reportCount}</Text><Text style={styles.levelHint}>Bu cihazdan görünen yakın çağrı sayısı</Text></View>
...
class AppErrorBoundary extends React.Component<{ children: React.ReactNode }, { error: string | null }> {
  ...
  render() {
    if (this.state.error) {
      return <SafeAreaView style={{ flex: 1, backgroundColor: CREAM, padding: 24, justifyContent: 'center' }}>
...
const styles = StyleSheet.create({
  ...
  caseCard: { backgroundColor: '#0E211C', borderRadius: 14, padding: 12, marginBottom: 12, flexDirection: 'row', borderWidth: 1, borderColor: '#29483E', shadowColor: '#000', shadowOpacity: .12, shadowRadius: 8, shadowOffset: { width: 0, height: 4 } }, caseImage: { width: 82, minHeight: 104, borderRadius: 10, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }, casePhoto: { width: '100%', height: '100%', position: 'absolute' }, verified: { position: 'absolute', bottom: 7, right: 7, width: 18, height: 18, borderRadius: 9, backgroundColor: '#4BA67E', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#18372E' }, caseBody: { flex: 1, paddingLeft: 13, paddingVertical: 2 }, caseTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 8 }, caseName: { fontSize: 16, fontWeight: '900', color: GREEN }, distance: { fontSize: 10, color: '#6E7974', fontWeight: '700' }, caseType: { fontSize: 11, lineHeight: 16, color: '#77807B', marginTop: 3 }, urgencyRow: { flexDirection: 'row', gap: 4, marginTop: 10 }, urgencyDot: { width: 18, height: 4, borderRadius: 2, backgroundColor: '#29483E' }, caseBottom: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 9 }, time: { fontSize: 10, color: '#949B97' }, miniButton: { minWidth: 74, minHeight: 30, backgroundColor: CORAL, paddingVertical: 7, paddingHorizontal: 12, borderRadius: 6, alignItems: 'center', justifyContent: 'center' }, miniButtonDone: { backgroundColor: MINT }, miniButtonText: { fontSize: 10, fontWeight: '800', color: '#071612' }, emptyCard: { backgroundColor: '#0E211C', borderRadius: 14, padding: 24, alignItems: 'center', marginBottom: 12, borderWidth: 1, borderColor: '#29483E' }, emptyTitle: { color: GREEN, fontWeight: '900', fontSize: 16, marginTop: 10 }, emptyText: { color: '#7A837F', textAlign: 'center', lineHeight: 18, fontSize: 12, marginTop: 6 }, outlineButton: { borderWidth: 1, borderColor: CORAL, borderRadius: 8, paddingVertical: 10, paddingHorizontal: 18, marginTop: 14, minWidth: 120, alignItems: 'center' }, outlineButtonText: { color: CORAL, fontWeight: '800' }, trustCard: { marginTop: 8, backgroundColor: '#102A24', borderRadius: 8, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, borderLeftWidth: 3, borderLeftColor: CORAL }, trustIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#17382F', alignItems: 'center', justifyContent: 'center' }, trustTitle: { fontSize: 13, fontWeight: '900', color: GREEN }, trustText: { fontSize: 10, lineHeight: 15, color: '#91AEA3', marginTop: 3 },
  navSafe: { backgroundColor: '#0E211C', borderTopWidth: 1, borderTopColor: '#29483E' }, nav: { height: 67, flexDirection: 'row', alignItems: 'center' }, navItem: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 3 }, navLabel: { fontSize: 9, fontWeight: '700', color: '#91AEA3' }, addButton: { width: 50, height: 50, borderRadius: 25, backgroundColor: CORAL, alignItems: 'center', justifyContent: 'center', marginTop: -25, shadowColor: CORAL, shadowOpacity: .32, shadowRadius: 10, shadowOffset: { width: 0, height: 5 } },
  mapSafe: { position: 'absolute', top: 0, left: 0, right: 0, zIndex: 3 }, mapHeader: { paddingHorizontal: 18, paddingTop: 10, flexDirection: 'row', justifyContent: 'space-between' }, locationPill: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 15, borderRadius: 10, backgroundColor: '#0E211C', maxWidth: SCREEN - 130 }, locationText: { fontSize: 12, fontWeight: '800', color: GREEN }, realMapWrap: { flex: 1, overflow: 'hidden' }, realMap: { flex: 1 }, mapPin: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#0E211C', alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: '#29483E', shadowColor: '#000', shadowOpacity: .18, shadowRadius: 6, shadowOffset: { width: 0, height: 3 } }, mapPinActive: { backgroundColor: CORAL, transform: [{ scale: 1.12 }] }, youMarkerReal: { width: 26, height: 26, borderRadius: 13, backgroundColor: 'rgba(90,209,176,.24)', alignItems: 'center', justifyContent: 'center' }, youCore: { width: 12, height: 12, borderRadius: 6, backgroundColor: '#5AD1B0', borderWidth: 2, borderColor: '#17382F' }, mapLoadingOverlay: { position: 'absolute', left: 18, right: 18, top: '45%', backgroundColor: 'rgba(7,22,18,.94)', borderRadius: 8, padding: 14, flexDirection: 'row', gap: 10, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#29483E' }, mapLoadingText: { fontSize: 11, fontWeight: '700', color: GREEN }, mapSheet: { backgroundColor: CREAM, padding: 15, borderTopLeftRadius: 28, borderTopRightRadius: 28 }, handle: { width: 38, height: 4, borderRadius: 2, backgroundColor: '#35584D', alignSelf: 'center', marginBottom: 10 }, mapCount: { fontSize: 15, fontWeight: '900', color: GREEN, marginBottom: 10 },
  formScroll: { padding: 22, paddingBottom: 36 }, formHeader: { marginTop: 10 }, formTitle: { fontSize: 34, lineHeight: 39, fontWeight: '900', letterSpacing: -1.1, color: GREEN }, formSub: { fontSize: 13, color: '#91AEA3', marginTop: 8 }, steps: { flexDirection: 'row', gap: 7, marginVertical: 27 }, step: { height: 5, flex: 1, borderRadius: 3, backgroundColor: '#F0DED2' }, stepActive: { backgroundColor: CORAL }, fieldLabel: { fontSize: 10, fontWeight: '900', letterSpacing: 1.2, color: '#68736E', marginBottom: 10, marginTop: 5 }, needGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginBottom: 23 }, needCard: { width: '48%', height: 92, borderRadius: 34, backgroundColor: '#0E211C', borderWidth: 1, borderColor: '#29483E', padding: 15, justifyContent: 'space-between' }, needCardActive: { backgroundColor: GREEN, borderColor: GREEN }, needText: { fontSize: 13, fontWeight: '900', color: GREEN }, textarea: { height: 120, backgroundColor: '#0E211C', borderRadius: 18, padding: 15, fontSize: 13, lineHeight: 19, color: GREEN, textAlignVertical: 'top', borderWidth: 1, borderColor: '#29483E' }, primaryButton: { marginTop: 24, minHeight: 55, minWidth: 210, borderRadius: 17, backgroundColor: CORAL, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 10, paddingHorizontal: 18 }, primaryButtonText: { fontSize: 14, fontWeight: '900', color: '#fff' }, backText: { textAlign: 'center', padding: 15, color: GREEN, fontWeight: '700' }, locationCard: { backgroundColor: '#0E211C', borderRadius: 18, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 25 }, locationMap: { width: 58, height: 58, borderRadius: 14, backgroundColor: '#17382F', alignItems: 'center', justifyContent: 'center' }, locationTitle: { fontSize: 13, fontWeight: '900', color: GREEN }, locationSub: { fontSize: 11, lineHeight: 15, color: '#91AEA3', marginTop: 3 }, photoActions: { flexDirection: 'row', gap: 10 }, photoAction: { flex: 1, height: 120, borderRadius: 20, borderWidth: 1.5, borderStyle: 'dashed', borderColor: '#AEB7B2', alignItems: 'center', justifyContent: 'center', padding: 14 }, photoTitle: { fontSize: 13, fontWeight: '900', color: GREEN, marginTop: 8 }, photoSub: { fontSize: 10, lineHeight: 15, textAlign: 'center', color: '#91AEA3', marginTop: 8 }, photoPreviewWrap: { height: 210, borderRadius: 20, overflow: 'hidden' }, photoPreview: { width: '100%', height: '100%' }, removePhoto: { position: 'absolute', top: 10, right: 10, width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(0,0,0,.65)', alignItems: 'center', justifyContent: 'center' }, reviewCard: { backgroundColor: '#0E211C', borderRadius: 40, padding: 20 }, reviewTitle: { fontSize: 20, fontWeight: '900', color: GREEN, marginBottom: 14 }, infoRow: { flexDirection: 'row', gap: 13, alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#29483E' }, infoLabel: { fontSize: 10, color: '#848C88' }, infoValue: { fontSize: 13, fontWeight: '800', color: GREEN, marginTop: 2 }, safetyNote: { backgroundColor: MINT, borderRadius: 14, padding: 12, flexDirection: 'row', gap: 10, marginTop: 15 }, safetyNoteText: { flex: 1, fontSize: 10, lineHeight: 15, color: '#526B63' }, successWrap: { flex: 1, alignItems: 'center', justifyContent: 'center', padding: 36 }, successCircle: { width: 90, height: 90, borderRadius: 45, backgroundColor: '#49A27B', alignItems: 'center', justifyContent: 'center' }, successTitle: { fontSize: 28, fontWeight: '900', color: GREEN, marginTop: 25 }, successText: { fontSize: 13, lineHeight: 20, textAlign: 'center', color: '#91AEA3', marginTop: 10 },
  ...
  statusRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 }, statusDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#35584D' }, statusDotActive: { backgroundColor: '#55A47D' }, statusText: { fontSize: 9, fontWeight: '800', color: '#6F7D76' }, privacyChoiceRow: { gap: 8, marginBottom: 22 }, privacyChoice: { minHeight: 70, borderRadius: 12, borderWidth: 1, borderColor: '#29483E', backgroundColor: '#0E211C', padding: 13, flexDirection: 'row', gap: 10, alignItems: 'center' }, privacyChoiceActive: { backgroundColor: '#17382F', borderColor: CORAL }, privacyChoiceTitle: { color: GREEN, fontSize: 12, fontWeight: '900' }, privacyChoiceText: { color: '#91AEA3', fontSize: 9, lineHeight: 14, marginTop: 2 }, duplicateWarning: { marginTop: 14, borderRadius: 8, padding: 12, backgroundColor: '#342917', flexDirection: 'row', gap: 10, alignItems: 'flex-start' }, duplicateTitle: { color: '#F2BC68', fontWeight: '900', fontSize: 11 }, duplicateText: { color: '#C9AA78', fontSize: 9, lineHeight: 14, marginTop: 3 },
  termsBox: { marginTop: 2, marginBottom: 12, backgroundColor: '#102A24', borderRadius: 14, padding: 12 }, checkRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 9 }, checkText: { flex: 1, color: GREEN, fontSize: 11, lineHeight: 17, fontWeight: '700' }, legalLinks: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginTop: 9, flexWrap: 'wrap' }, legalLink: { color: CORAL, fontSize: 11, fontWeight: '800', textDecorationLine: 'underline' }, legalDot: { color: '#89928E' }, moderationRow: { flexDirection: 'row', alignItems: 'center', gap: 7, marginTop: 7 }, moderationText: { color: '#7D8581', fontSize: 9, fontWeight: '800', textDecorationLine: 'underline' }, moderationSep: { color: '#B4BAB7', fontSize: 9 }, termsGate: { flex: 1, backgroundColor: CREAM }, termsGateContent: { flexGrow: 1, padding: 28, justifyContent: 'center', maxWidth: 620, width: '100%', alignSelf: 'center' }, termsGateTitle: { color: GREEN, fontSize: 31, lineHeight: 36, fontWeight: '900', marginTop: 22 }, termsGateBody: { color: '#65716C', fontSize: 13, lineHeight: 20, marginTop: 12 }, termsNotice: { backgroundColor: MINT, padding: 15, borderRadius: 16, marginTop: 18 }, termsNoticeText: { color: '#4E685F', fontSize: 11, lineHeight: 17 },
  profileTop: { alignItems: 'center', paddingVertical: 10 }, bigAvatar: { width: 82, height: 82, borderRadius: 41, backgroundColor: MINT, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#315247' }, bigAvatarText: { fontSize: 30, fontWeight: '900', color: GREEN }, profileName: { fontSize: 23, fontWeight: '900', color: GREEN, marginTop: 12 }, profileEmail: { fontSize: 11, color: '#7E8783', marginTop: 3 }, verifiedRow: { flexDirection: 'row', gap: 5, alignItems: 'center', marginTop: 7 }, verifiedText: { fontSize: 10, fontWeight: '700', color: '#59B98E' }, levelCard: { minHeight: 146, backgroundColor: '#102A24', borderRadius: 28, padding: 20, marginVertical: 18, borderLeftWidth: 4, borderLeftColor: CORAL, overflow: 'hidden' }, profileAnimal: { position: 'absolute', width: 105, height: 150, right: 12, bottom: -20, opacity: .9 }, levelSmall: { fontSize: 9, letterSpacing: 1.2, fontWeight: '800', color: '#8BA99E' }, levelTitle: { fontSize: 19, fontWeight: '900', color: GREEN, marginTop: 4 }, levelNumber: { position: 'absolute', right: 122, top: 17, fontSize: 32, fontWeight: '900', color: CORAL }, levelHint: { fontSize: 9, color: '#8BA99E', marginTop: 42, maxWidth: '58%' }, menuRow: { backgroundColor: 'transparent', minHeight: 62, paddingHorizontal: 6, paddingVertical: 14, borderRadius: 0, borderBottomWidth: 1, borderBottomColor: '#1F3A32', marginBottom: 0, flexDirection: 'row', alignItems: 'center', gap: 14 }, menuText: { flex: 1, fontSize: 13, fontWeight: '700', color: GREEN },
});
